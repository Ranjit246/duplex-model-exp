...
# Smoke-oriented public training example.
# This variant keeps the same finetuning target as the default example, but
# uses a more conservative DeepSpeed config with CPU offload.
#
...
OUTPUT_DIR="${OUTPUT_DIR:-output/moshiko-finetuned-cpuoffload-smoke}"
DEEPSPEED_CONFIG="${DEEPSPEED_CONFIG:-ds_configs/zero3-bf16-cpuoffload-smoke.json}"
