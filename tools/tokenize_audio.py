...
from kame.models import MimiModel, loaders
...
def _save_tokenized_audio(
    output_path: str, audio_ids_a: torch.Tensor, audio_ids_b: torch.Tensor
) -> None:
    try:
...
def worker(process_id: int, dialogue_names: list[str], args: argparse.Namespace):
    ...
    for dialogue_name in pbar:
        ...
        _save_tokenized_audio(output_path, audio_ids_A, audio_ids_B)
